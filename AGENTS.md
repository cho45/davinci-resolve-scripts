# DaVinci Resolve Scripting Guide for Agents & Developers (llms.txt)

This file serves as a comprehensive "How-To" guide, reference manual, and collection of gotchas for writing DaVinci Resolve scripts (Python/Lua) within this repository. Read this file before creating new plugins or scripts.

## 1. Environment & Setup

- **Placement**: Scripts should be placed under `src/Scripts/<PageName>/` (e.g., `Utility/`, `Edit/`, `Color/`). 
  - `Utility/` makes the script appear globally on the `Workspace > Scripts > MyScripts` menu.
- **Languages**: Lua 5.1 and Python (2.7 or >= 3.6) are supported.
- **Boilerplate**:
  To initialize the API connection in any script:
  ```lua
  -- Lua
...
## 2. Common Gotchas & "Do NOT Do This" (Critical)

DaVinci Resolve has several hidden behaviors and bugs. Always follow these rules:

### A. UI and Event Loop (`UIDispatcher`)
When creating a GUI window, do NOT rely on standard OS popups. Use Fusion's `UIManager` and `UIDispatcher`.
- **Never assign undeclared or nil values to UI event handlers**. Doing so will crash the UI with an `ontbl nil value` error.
- **Ensure you keep the `bmd.UIDispatcher(ui)` loop running** using `dispatcher:RunLoop()`. Always provide a clean exit using `dispatcher:ExitLoop()`.

### B. Clipboard Access
Do NOT use OS-specific shell commands like `io.popen("clip", "w")` to copy text to the clipboard. It introduces shift-JIS encoding issues on Windows and spawns visible CMD windows.
- **Use the native API**: `bmd.setclipboard("text")` (available from Fusion API).

### C. Timeline Frame Rates & Settings
Retrieving the Timeline frame rate `timeline:GetSetting("timelineFrameRate")` returns a string.
- Sometimes the setting includes drop-frame notation (e.g., `"29.97 DF"`, `"29.97 NDF"`).
- **Rule**: ALWAYS safely parse the string using string substitutions before converting to a number.
- **Rule**: If `fps` fails to parse, DO NOT fallback to 24fps silently. Throw an explicit error to the user or console.

```lua
local frameRateStr = timeline:GetSetting("timelineFrameRate")
local fps = tonumber(frameRateStr)
if not fps and frameRateStr then
    frameRateStr = string.gsub(frameRateStr, " DF", "")
    frameRateStr = string.gsub(frameRateStr, " NDF", "")
    fps = tonumber(frameRateStr)
end
```

### D. Finding Markers and Absolute VS Relative Timestamps
There are two ways to get markers:
1. **Timeline Markers**: `Timeline:GetMarkers()` 
   - These returns an associative table (Lua) / dict (Python) where the Key is the frame offset relative to `00:00` of the timeline.
   - You do NOT need to subtract `Timeline:GetStartFrame()` from these values. To get the time: `math.floor(frameId / fps + 0.5)`.
2. **Timeline Item (Clip) Markers**: `TimelineItem:GetMarkers()`
   - Tied to individual clips, their positions are relative to the source media. You must offset them against the timeline item's `GetStart()` position to figure out when they appear on the main timeline.

## 3. Creating UI Components (Lua Example)

Use the UI Dispatcher to build functional interfaces.
...
## 4. Testing and Debugging via CLI

When DaVinci Resolve is running, LLMs (Agents) and developers do NOT need to manually click through the UI to test APIs or debug state. You can query Resolve's state and run test scripts directly from the command line mapping to standard output.

**For Lua (using `fuscript.exe`):**
```powershell
...
```
*(You can pass multi-line or complex Lua snippets inside the `-x` block to check properties before writing the full plugin).*
...
**Rule for Agents**: Before guessing how an undocumented property behaves, or if you encounter ambiguity in the API, write a quick test script and execute it via `run_command` in powershell! This eliminates assumptions and fixes issues instantly.
